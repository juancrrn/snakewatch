class BotSnake extends Snake{
  ...
  constructor(){
    ...
    var initX = floor(random(6, numCols-1));
    var initY = floor(random(0, numRows-1));
    
    // Start with long body
    this.body[0] = createVector(initX, initY);
    this.body[1] = createVector(this.head.x -1, this.head.y);
    this.body[2] = createVector(this.head.x -2, this.head.y);
    this.body[3] = createVector(this.head.x -3, this.head.y);
    this.body[4] = createVector(this.head.x -4, this.head.y);
    ...
    if(this.checkCrash(this.head.x +this.xDir, this.head.y +this.yDir) || 
      ...
      if (this.xDir == 0){
        ...
        if(this.checkCrash(this.head.x -1, this.head.y) || 
          (random(0, 1) > 0.5 && !this.checkCrash(this.head.x +1, this.head.y))){
          newXDir = 1;
        }
      ...
      // Si se estaba moviendo en eje X (dcha o izq) -> mover en eje Y (arriba o abajo)
        ...
        if(this.checkCrash(this.head.x, this.head.y -1) || 
          (random(0, 1) > 0.5 && !this.checkCrash(this.head.x, this.head.y +1))){
          newYDir = 1;
        }
